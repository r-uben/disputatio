...
### Wave 2.7 — Expositional craft audit (new — Phase 2.7)

Three sub-waves running parallel to Phase 2 discovery and Phase 2.6 scope/framing. No dependencies between them. Authoritative protocols in `templates/exposition_triage.md`, `templates/exposition.md`, `templates/exposition_integrate.md`.

**Wave 2.7a — per-family triage** (3 tickets, parallel):

```json
{
  "exposition_triage_claude": {
    "id": "exposition_triage_claude", "type": "exposition_triage", "agent": "claude",
    "model": "sonnet", "family": "anthropic", "flags": {},
    "prompt_path": "_artifacts/prompts/exposition_triage_claude.md",
    "inputs": [
      "_paper/paper.md",
      "_artifacts/json/orient_claude.json",
      "_artifacts/json/holistic_claude.json"
    ],
    "outputs": ["_artifacts/json/exposition_triage_claude.json"],
    "depends_on": ["holistic_claude"],
    "status": "pending", "timeout_s": 1200
  },
  "exposition_triage_codex":  { "...agent=codex, model=gpt-5.4-mini, family=openai, depends_on=[holistic_codex]..." },
  "exposition_triage_gemini": { "...agent=gemini, model=gemini-3-flash-preview, family=google, depends_on=[holistic_gemini], output_format=json_stdout..." }
}
```

**Wave 2.7b — per-family audit** (3 tickets, parallel, depends on the family's own triage):

```json
{
  "exposition_claude": {
    "id": "exposition_claude", "type": "exposition", "agent": "claude",
    "model": "sonnet", "family": "anthropic", "flags": {},
    "prompt_path": "_artifacts/prompts/exposition_claude.md",
    "inputs": [
      "_paper/paper.md",
      "_artifacts/json/orient_claude.json",
      "_artifacts/json/holistic_claude.json",
      "_artifacts/json/exposition_triage_claude.json"
    ],
    "outputs": ["_artifacts/json/exposition_claude.json"],
    "depends_on": ["exposition_triage_claude"],
    "status": "pending", "timeout_s": 1500
  },
  "exposition_codex":  { "...agent=codex, model=gpt-5.4, family=openai, depends_on=[exposition_triage_codex]..." },
  "exposition_gemini": { "...agent=gemini, model=gemini-3.1-pro-preview, family=google, depends_on=[exposition_triage_gemini], output_format=json_stdout..." }
}
```

**Wave 2.7c — global integration** (1 ticket, Claude-typed inline, depends on all 3 audits):

```json
{
  "exposition_integrate": {
    "id": "exposition_integrate", "type": "exposition_integrate", "agent": "claude",
    "model": "opus", "family": "anthropic", "flags": {},
    "prompt_path": "_artifacts/prompts/exposition_integrate.md",
    "inputs": [
      "_artifacts/json/exposition_claude.json",
      "_artifacts/json/exposition_codex.json",
      "_artifacts/json/exposition_gemini.json"
    ],
    "outputs": [
      "_artifacts/json/exposition_ledger.json",
      "_artifacts/json/exposition_queue.json"
    ],
    "depends_on": ["exposition_claude", "exposition_codex", "exposition_gemini"],
    "status": "pending", "timeout_s": 1200
  }
}
```
...
After Phase 2.7c emits `_artifacts/json/exposition_queue.json`, the orchestrator emits one calibrate ticket per cluster in the queue. Each ticket runs `templates/exposition_calibration.md` against its cluster.

```json
{
  "calibrate_exposition_EXP_C001": {
    "id": "calibrate_exposition_EXP_C001", "type": "calibrate_exposition", "agent": "codex",
    "model": "gpt-5.4", "family": "openai", "flags": {},
    "prompt_path": "_calibration/prompts/exposition_EXP_C001.md",
    "inputs": ["_artifacts/json/exposition_queue.json", "_paper/paper.md"],
    "outputs": ["_calibration/exposition/EXP_C001.json"],
    "depends_on": ["exposition_integrate"],
    "status": "pending", "timeout_s": 900,
    "max_attempts": 2
  }
}
...
Run as a sub-DAG: `agent-ctl run-dag _calibration/exposition_tickets.json --cwd <paper> --concurrent 3`. The full-Codex (gpt-5.4 non-mini) is used because the anti-pedantry guard requires more than a rubric-bounded mini read — same rationale as the post-polish upgraded re-annotator in `templates/calibrate.md`.

After all tickets complete, the orchestrator aggregates per-cluster outputs into `_artifacts/json/exposition_calibrated_rows.json`. Rows with `verdict: supported_editorial` merge into `panel_rows_candidates.json` alongside main, gap (3g), validity (3v), and scope-framing (3s) rows.

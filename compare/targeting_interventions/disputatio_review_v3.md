...
**Method**: Disputatio (seven-method dialectic debate), v3
...
### 14. Inline definition of $\alpha_\ell$ drops the square from the denominator

**Status**: Pending

**Quote**:
> the eigenvalue corresponding to $\bm{u}^{\ell}(\bm{G})$ (via $\alpha_{\ell}=\frac{1}{1-\beta\lambda_{\ell}}$)

**Feedback**:
Theorem 1 defines $\alpha_\ell = 1/(1-\beta\lambda_\ell)^2$. The inline restatement in the paragraph interpreting the theorem writes $\alpha_\ell = 1/(1-\beta\lambda_\ell)$, dropping the exponent 2 from the denominator. This is not the introduction of a different quantity -- the sentence is explicitly restating the same $\alpha_\ell$ that appears in equation (5). The missing square means that all subsequent informal references to $\alpha_\ell$ in this paragraph are keyed to the wrong expression. Since $\alpha_\ell$ governs the amplification ordering that drives the paper's main results, this error risks misleading a reader who relies on the inline definition rather than the formal display. Restore the exponent: $\alpha_\ell = 1/(1-\beta\lambda_\ell)^2$.

---

### 15. SVD equilibrium formula has a spurious square in OA3.2
...
**Feedback**:
The equilibrium condition $\mathbf{M}\mathbf{a}^* = \mathbf{b}$ with $\mathbf{M} = \mathbf{U}\mathbf{S}\mathbf{V}^\top$ gives $\mathbf{S}\mathbf{V}^\top\mathbf{a}^* = \mathbf{U}^\top\mathbf{b}$, i.e., $s_\ell \underline{a}_\ell^* = \underline{b}_\ell$ component-wise, so $\underline{a}_\ell^* = \underline{b}_\ell / s_\ell$. The displayed formula writes $b_\ell^2$ (squared) instead of $\underline{b}_\ell$ (linear), and also drops the underline notation. Since the equilibrium is linear in $\mathbf{b}$, squaring $b_\ell$ is algebraically wrong -- it changes the dimensionality of the expression and would make the subsequent welfare decomposition (which depends on $\underline{a}_\ell^*$ being linear in $\underline{b}_\ell$) inconsistent. Replace $b_\ell^2$ with $\underline{b}_\ell$.

---

### 16. Beauty contest FOC has $\tilde{b}_i$ where it should be $\tilde{\beta}$ in OA2.2
...
**Feedback**:
Differentiating $U_i = a_i(\tilde{b}_i + \tilde{\beta}\sum_j g_{ij}a_j) - \frac{1}{2}a_i^2 - \frac{\gamma}{2}\sum_j g_{ij}(a_j - a_i)^2$ with respect to $a_i$ and using $\sum_j g_{ij} = 1$ gives $a_i = \tilde{b}_i/(1+\gamma) + (\tilde{\beta}+\gamma)/(1+\gamma)\sum_j g_{ij}a_j$. The coefficient of $\sum g_{ij}a_j$ should be $(\tilde{\beta}+\gamma)/(1+\gamma)$, not $(\tilde{b}_i+\gamma)/(1+\gamma)$. This matters because $\tilde{b}_i$ is agent-specific: if it appeared in the slope of the best response, the interaction coefficient $\beta$ would be heterogeneous across agents, contradicting the model's structure and the subsequent identification $\beta = (\tilde{\beta}+\gamma)/(1+\gamma)$. Replace $\tilde{b}_i$ with $\tilde{\beta}$ in the numerator of the second fraction.

---

### 17. Example 2 change of variables is circular: $b_i = [\tau - b_i]/2$

**Status**: Pending

**Quote**:
> Performing the change of variables $b_{i}=[\tau-b_{i}]/2$ and $\beta=-\tilde{\beta}/2$

**Feedback**:
The variable $b_i$ is being defined by this substitution, so it cannot also appear on the right-hand side. Solving $b_i = (\tau - b_i)/2$ as written yields $b_i = \tau/3$, eliminating all individual heterogeneity. The correct formula is $b_i = (\tau - \tilde{b}_i)/2$, as confirmed by: (i) the status quo formula in the same sentence, $\hat{b}_i = (\tau - \tilde{b}_i)/2$, which uses $\tilde{b}_i$; and (ii) deriving the FOC directly, which gives $a_i = (\tau - \tilde{b}_i)/2 + (-\tilde{\beta}/2)\sum_j g_{ij}a_j$, matching $b_i = (\tau - \tilde{b}_i)/2$. This is likely an OCR artifact (the tilde may have been lost), but the circular definition as printed is incorrect. Replace $b_i$ on the right-hand side with $\tilde{b}_i$.

---

### 18. Lemma OA1 states $u_i^1(\mathbf{G}) = \sqrt{n}$ instead of $1/\sqrt{n}$

**Status**: Pending

**Quote**:
> $\lambda_{1}(\bm{G})=1$ and $u_{i}^{1}(\bm{G})=\sqrt{n}$ for all $i$

**Feedback**:
Under Assumption OA1, $\sum_j g_{ij} = 1$ for all $i$, so $\mathbf{G}\mathbf{1} = \mathbf{1}$, making the all-ones vector an eigenvector with eigenvalue 1. The normalized eigenvector has entries $1/\sqrt{n}$, not $\sqrt{n}$. The vector $(\sqrt{n}, \ldots, \sqrt{n})$ has norm $n$, violating the unit-norm convention established in Fact 1. With entries $1/\sqrt{n}$, the norm is $\sqrt{n \cdot 1/n} = 1$, satisfying the normalization. Replace $\sqrt{n}$ with $1/\sqrt{n}$. Note that Part 3 of the same lemma, which writes $\sum_i a_i^* = (\sqrt{n}/(1-\beta))\underline{b}_1 = \sqrt{n\alpha_1}\underline{b}_1$, is consistent with $u_i^1 = 1/\sqrt{n}$ (since $\underline{b}_1 = \mathbf{u}^1 \cdot \mathbf{b} = (1/\sqrt{n})\sum_i b_i$, giving $\sum_i a_i^* = \sqrt{n} \cdot (1/\sqrt{n})\sum_i b_i/(1-\beta) = \sum_i b_i/(1-\beta)$, which is correct).

---

### 19. Proposition 2 discussion swaps denominator sign for substitutes case
...
**Feedback**:
Proposition 2 part 2 states the budget condition as $C > (2\|\hat{\mathbf{b}}\|^2/\epsilon)(\alpha_{n-1}/(\alpha_n - \alpha_{n-1}))^2$. For $\beta < 0$, $\alpha_n > \alpha_{n-1}$, so $\alpha_n - \alpha_{n-1} > 0$ is the correct (positive) denominator. The discussion paragraph writes $\alpha_{n-1}/(\alpha_{n-1} - \alpha_n)$, which reverses the subtraction order and produces a negative denominator. Because the ratio is squared in the formal bound, the sign reversal does not change the actual condition on $C$, but it creates a notational inconsistency with the proposition statement. Additionally, the verbal claim that this term is "small" when the bottom gap is small appears to be the opposite of the intended meaning (a small gap makes the bound larger, hence harder to satisfy, as stated correctly two paragraphs later). Align the inline expression with the proposition's $\alpha_n - \alpha_{n-1}$ and clarify the direction of the comparative static.

---

### 20. Additional proof-level errors in the online appendix
...
**Quote (ii)**:
> $\mu$ as a function of $x_{1}\in\left[-C/\hat{b}_{1},C\hat{b}_{1}\right]$ is U-shaped

**Feedback**:
Two further errors in the appendix proofs:

(i) In the proof of Proposition 2 (welfare section, final line), $W^2$ should be $W^s$. The quantity $W^s$ (welfare under the simple intervention) is defined and used throughout the proof; $W^2$ is never defined. This is likely an OCR or typesetting error where the superscript $s$ was rendered as $2$.

(ii) In the paragraph after Lemma OA2 in the proof of Theorem OA1, the domain of $\mu$ as a function of $x_1$ is stated as $x_1 \in [-C/\hat{b}_1, C\hat{b}_1]$. The budget constraint $\hat{b}_1^2 x_1^2 \leq C$ gives $|x_1| \leq \sqrt{C}/\hat{b}_1$. Lemma OA2 part 1 correctly uses $\sqrt{C}/\hat{b}_1$ as the endpoint. The summary paragraph drops the square root and also garbles the upper bound notation (writing $C\hat{b}_1$ instead of $C/\hat{b}_1$ or $\sqrt{C}/\hat{b}_1$). Replace the domain with $[-\sqrt{C}/\hat{b}_1, \sqrt{C}/\hat{b}_1]$.

Both errors are confined to the proofs and do not affect the stated results, but correcting them would remove potential confusion for readers verifying the arguments.
